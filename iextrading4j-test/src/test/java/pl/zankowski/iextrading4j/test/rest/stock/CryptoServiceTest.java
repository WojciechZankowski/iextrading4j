...
public class CryptoServiceTest extends BaseRestServiceTest {
    ...
    public void cryptoServiceTest() {
        stubFor(get(urlEqualTo("/stock/market/crypto"))
                .withHeader("Accept", equalTo("application/json"))
                .willReturn(aResponse()
                        .withStatus(200)
                        .withHeader("Accept", "application/json")
                        .withHeader("Content-Type", "application/json")
                        .withBodyFile("rest/stock/CryptoResponse.json")));

        final List<Quote> quotes = iexTradingClient.executeRequest(new CryptoRequestBuilder()
                .build());
